...
def confirm_rent_view(request, slug):
    try:
        ...
        if b.book_amount <= 0:
            messages.warning(
                request, f'Вы не можете взять эту книгу')
            return redirect('bookDetail', slug=b.slug)
    except Book.DoesNotExist:
        raise Http404("У нас нет этой книги")
    ...
    try:
        ...
        if b:
            if b.book_amount > 0:
                ...
                log_history.save()
                messages.success(
                    request, f'Вы берете книгу: {b.title}')
            else:
                messages.warning(
                    request, f'Вы не можете взять эту книгу')
                return redirect('bookDetail', slug=b.slug)
...
def return_book_view(request, slug):
    try:
        ...
        if b:
            ...
            log_history.delete()
            messages.success(
                request, f'Вы не можете взять напрокат эту книгу: {b.title}')
        else:
            messages.warning(
...
def rate_book_view(request, slug, rating):
    try:
        ...
        if b and not(BookReview.objects.filter(user=request.user).filter(book=b)):
            ...
            b.save()
            messages.success(
                request, f'Вы оценили книгу: {b.title}')

        else:
            messages.warning(
                request, f'Вы уже оценили эту книгу')
        return redirect('bookDetail', slug=b.slug)
    except Book.DoesNotExist:
        raise Http404("Book is unavailable")
    return redirect('bookDetail', slug=b.slug)
...
def contact_form(request):
    ...
    if request.user.is_authenticated:
        ...
        form.fields['message'].widget.attrs['placeholder'] = 'Напишите свое сообщение здесь'
